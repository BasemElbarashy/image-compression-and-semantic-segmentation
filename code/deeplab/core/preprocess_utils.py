...
def randomly_scale_image_and_label(image, label=None, depth=None, scale=1.0):
  ...
  if depth is not None:
      depth = tf.squeeze(tf.image.resize_bilinear(
          tf.expand_dims(depth, 0),
          new_dim,
          align_corners=True), [0])
...
def resize_to_range(image,
  ...
  with tf.name_scope(scope, 'resize_to_range', [image]):
    ...
    if depth is not None:
      if label_layout_is_chw:
        # Input label has shape [channel, height, width].
        resized_depth = tf.expand_dims(depth, 3)
        resized_depth = tf.image.resize_nearest_neighbor(
            resized_depth, new_size, align_corners=align_corners)
        resized_depth = tf.squeeze(resized_depth, 3)
      else:
        # Input label has shape [height, width, channel].
        resized_depth = tf.image.resize_images(
            depth, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
            align_corners=align_corners)
      new_tensor_list.append(resized_depth)
    else:
      new_tensor_list.append(None)
