...
def _convert_dataset(dataset_split):
  ...
  depth_reader = build_data.ImageReader('png', channels=1)
  ...
  for shard_id in range(_NUM_SHARDS):
    ...
    with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
      ...
      for i in range(start_idx, end_idx):
        ...
        # Read depth annotation
        depth_data = tf.gfile.FastGFile(depth_files[i], 'rb').read()
        depth_height, depth_width = label_reader.read_image_dims(depth_data)
        if height != depth_height or width != depth_width:
          raise RuntimeError('Shape mismatched between image and depth.')
        ...
        example = build_data.image_seg_to_tfexample(
            image_data, filename, height, width, seg_data, depth_data)
