...
def replace_im_w_noise(im):
    x_n = tf.random.normal(shape= tf.shape(im), mean= 0.5, stddev=0.25, dtype=tf.float32)
    x_n = tf.math.abs(x_n)
    return x_n
...
def load_image_rgbd(filename):
  ...
  image_d = replace_im_w_noise(image_d)
...
def get_importance_map(im_path):
  ...
  blurred = cv2.GaussianBlur(rgb_im, (3, 3), 1)
  gray = cv2.cvtColor(blurred, cv2.COLOR_BGR2GRAY)
  high_thresh, thresh_im = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
  high_thresh = 1 * high_thresh
  low_thresh = 0.5 * high_thresh
  edges_im = cv2.Canny(gray, low_thresh, high_thresh)
